...
const UpdateDomModal = ({ updateDom, setUpdateDom, refetch }) => {
    const handleUpdateDom = event => {
        ...
        fetch(`https://floating-peak-19260.herokuapp.com/domLessons/${updateDom._id}`, {
            method: "PUT",
            headers: {
    ...
    return (
        <div>
            <input type="checkbox" id="update-Dom-modal" className="modal-toggle" />
            <div className="modal modal-bottom sm:modal-middle">
                <div className="modal-box">
                    <h3 className="font-bold text-slate-700 text-3xl text-center mb-2">Update Content</h3>
                    <form onSubmit={handleUpdateDom} className='w-full text-center'>
                    ...
                    <div className="modal-action">
                        <label htmlFor="update-Dom-modal" className="btn btn-sm">Cancel</label>
                    </div>
                </div>
            </div>
        </div>
