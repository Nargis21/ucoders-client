...
const UpdateNodeModal = ({ updateNode, setUpdateNode, refetch }) => {
    const handleUpdateNode = event => {
        ...
        fetch(`https://ucoders-server.vercel.app/nodeLessons/${updateNode._id}`, {
            method: "PUT",
            headers: {
    ...
    return (
        <div>
            <input type="checkbox" id="update-node-modal" className="modal-toggle" />
            <div className="modal modal-bottom sm:modal-middle">
                <div className="modal-box">
                    <h3 className="font-bold text-slate-700 text-3xl text-center mb-2">Update Content</h3>
                    <form onSubmit={handleUpdateNode} className='w-full text-center'>
                    ...
                    <div className="modal-action">
                        <label htmlFor="update-node-modal" className="btn btn-sm">Cancel</label>
                    </div>
                </div>
            </div>
        </div>
