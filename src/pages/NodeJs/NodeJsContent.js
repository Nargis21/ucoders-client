...
const NodeContent = () => {
    ...
    useEffect(() => {
        fetch(`https://floating-peak-19260.herokuapp.com/nodeLessons/${id}`)
            .then(res => res.json())
    ...
    return (
        <div className='lg:px-4 px-2 lg:pt-4 text-slate-200'>
            ...
            <div>
                <h1 className='lg:text-3xl text-2xl font-semibold py-4'>{nodeLesson.subTitle}</h1>
                <div className=''>
                    <h1 className='text-xl'>{nodeLesson.description}</h1>
                </div>
            </div>
            <div className='lg:m-4 p-4 my-4 bg-slate-500 rounded'>
                <h1 className='text-3xl font-semibold py-4'>Example:</h1>
                <img src={nodeLesson.exampleImg} alt="" />
            ...
            <div className='bg-amber-100 m-6 p-6 text-black'>
                <h1> <span className='font-bold'>Note:</span> {nodeLesson.note}</h1>
            </div>
        </div>
    );
};
